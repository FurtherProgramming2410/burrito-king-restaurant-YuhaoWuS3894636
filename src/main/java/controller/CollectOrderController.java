...
public class CollectOrderController {
    ...
    public CollectOrderController(Stage stage, Model model) {
        this.stage = stage;
        this.model = model;
    }
    ...
    private void collectOrder() {
        ...
        try {
            ...
            if ("cancelled".equalsIgnoreCase(order.getStatus())) {
                ...
                alert.setHeaderText(null);
                alert.setContentText("The order has been cancelled and cannot be collected.");
                alert.showAndWait();
                return;
            }

            boolean updated = model.getUserDao().updateOrderStatus(orderId, "collected");
            if (updated) {
                Alert alert = new Alert(Alert.AlertType.INFORMATION);
                alert.setTitle("Success");
                alert.setHeaderText(null);
                alert.setContentText("Order has been marked as collected.");
                alert.showAndWait();
                refreshOrderList();
            } else {
                Alert alert = new Alert(Alert.AlertType.ERROR);
